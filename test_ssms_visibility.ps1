...
Write-Host ""
Write-Host "Expected result: Should return 'master' and 'tsql_wasm'"
Write-Host ""

# Build and run the test
Write-Host "Building test..." -ForegroundColor Yellow
cargo build --example test_ssms_full -p tsql_core 2>&1 | Out-Null

Write-Host "Running test..." -ForegroundColor Yellow
$output = cargo run --example test_ssms_full -p tsql_core 2>&1
