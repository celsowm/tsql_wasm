...
#[tokio::test]
#[ignore]
async fn test_cte_simple_compare() {
    let mut client = connect().await;
    let mut engine = Engine::new();

    exec_sql(&mut client, "DROP TABLE IF EXISTS t_cte_src").await;
    exec_sql(&mut client, "CREATE TABLE t_cte_src (id INT, val INT)").await;
    engine_exec(&mut engine, "CREATE TABLE t_cte_src (id INT, val INT)");

    exec_sql(&mut client, "INSERT INTO t_cte_src VALUES (1, 100), (2, 200), (3, 300)").await;
    ...
    for (i, (sql_row, eng_row)) in sql_rows.iter().zip(engine_result.rows.iter()).enumerate() {
        ...
        assert_eq!(sql_row, &eng_strings, "CTE linha {} diverge", i);
    }

    exec_sql(&mut client, "DROP TABLE t_cte_src").await;
}
...
#[tokio::test]
#[ignore]
async fn test_cte_join_compare() {
    let mut client = connect().await;
    let mut engine = Engine::new();

    for &t in &["t_cte_a", "t_cte_b"] {
        exec_sql(&mut client, &format!("DROP TABLE IF EXISTS {}", t)).await;
    }

    exec_sql(&mut client, "CREATE TABLE t_cte_a (id INT, name VARCHAR(10))").await;
    exec_sql(&mut client, "CREATE TABLE t_cte_b (id INT, score INT)").await;
    engine_exec(&mut engine, "CREATE TABLE t_cte_a (id INT, name VARCHAR(10))");
    engine_exec(&mut engine, "CREATE TABLE t_cte_b (id INT, score INT)");

    exec_sql(&mut client, "INSERT INTO t_cte_a VALUES (1, 'Alice'), (2, 'Bob')").await;
    exec_sql(&mut client, "INSERT INTO t_cte_b VALUES (1, 90), (2, 80)").await;
    engine_exec(&mut engine, "INSERT INTO t_cte_a VALUES (1, 'Alice'), (2, 'Bob')");
    engine_exec(&mut engine, "INSERT INTO t_cte_b VALUES (1, 90), (2, 80)");

    let sql = "WITH cte AS (SELECT * FROM t_cte_b WHERE score > 85) SELECT a.name, c.score FROM t_cte_a a JOIN cte c ON a.id = c.id ORDER BY a.name";
    let (_, sql_rows) = query_sql(&mut client, sql).await;
    let engine_result = engine_exec(&mut engine, sql).unwrap();
    ...
    for (i, (sql_row, eng_row)) in sql_rows.iter().zip(engine_result.rows.iter()).enumerate() {
        let eng_strings = values_to_strings(eng_row);
        assert_eq!(sql_row, &eng_strings, "CTE JOIN linha {} diverge", i);
    }

    exec_sql(&mut client, "DROP TABLE t_cte_a").await;
    exec_sql(&mut client, "DROP TABLE t_cte_b").await;
}