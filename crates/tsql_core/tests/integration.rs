...
fn test_join() {
    ...
    // Testar JOIN
    let stmt = parse_sql("SELECT o.id, u.name FROM orders o INNER JOIN users u ON o.user_id = u.id").unwrap();
    let result = engine.execute(stmt).unwrap().unwrap();
    assert_eq!(result.columns, vec!["id", "name"]);
...
fn test_group_by() {
    ...
    engine.execute(parse_sql("INSERT INTO sales (id, category, amount) VALUES (1, 'A', 100)").unwrap()).unwrap();
    engine.execute(parse_sql("INSERT INTO sales (id, category, amount) VALUES (2, 'A', 200)").unwrap()).unwrap();
    engine.execute(parse_sql("INSERT INTO sales (id, category, amount) VALUES (3, 'B', 150)").unwrap()).unwrap();
...
fn test_order_by() {
    ...
    engine.execute(parse_sql("CREATE TABLE items (id INT NOT NULL PRIMARY KEY, name VARCHAR(50))").unwrap()).unwrap();
    engine.execute(parse_sql("INSERT INTO items (id, name) VALUES (3, 'C')").unwrap()).unwrap();
    engine.execute(parse_sql("INSERT INTO items (id, name) VALUES (1, 'A')").unwrap()).unwrap();
    engine.execute(parse_sql("INSERT INTO items (id, name) VALUES (2, 'B')").unwrap()).unwrap();
...
fn test_identity() {
    ...
    engine.execute(parse_sql("CREATE TABLE auto (id INT IDENTITY(1,1) NOT NULL PRIMARY KEY, data VARCHAR(50))").unwrap()).unwrap();
    engine.execute(parse_sql("INSERT INTO auto (data) VALUES ('first')").unwrap()).unwrap();
    engine.execute(parse_sql("INSERT INTO auto (data) VALUES ('second')").unwrap()).unwrap();
...
fn test_default_values() {
    ...
    engine.execute(parse_sql("CREATE TABLE def (id INT NOT NULL PRIMARY KEY, val INT DEFAULT 42)").unwrap()).unwrap();
    engine.execute(parse_sql("INSERT INTO def (id) VALUES (1)").unwrap()).unwrap();
...
fn test_top() {
    ...
    engine.execute(parse_sql("CREATE TABLE nums (id INT NOT NULL PRIMARY KEY)").unwrap()).unwrap();
    for i in 1..=10 {
        engine.execute(parse_sql(&format!("INSERT INTO nums (id) VALUES ({})", i)).unwrap()).unwrap();
    }
