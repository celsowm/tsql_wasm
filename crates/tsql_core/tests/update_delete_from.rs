...
fn test_update_from_basic() {
    ...
    exec(
        &mut e,
        "UPDATE target SET val = new_val FROM target INNER JOIN source ON target.id = source.id",
    );
...
fn test_update_from_with_alias() {
    ...
    exec(
        &mut e,
        "UPDATE t SET t.val = s.new_val FROM t INNER JOIN s ON t.id = s.id",
    );
...
fn test_delete_from_basic() {
    ...
    exec(
        &mut e,
        "DELETE FROM target FROM target INNER JOIN source ON target.id = source.id",
    );
