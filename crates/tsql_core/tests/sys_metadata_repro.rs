...
fn test_sys_metadata_fidelity() {
    ...
    exec(
        &mut engine,
        "CREATE PROCEDURE dbo.MyProc @p1 INT, @p2 VARCHAR(10) OUTPUT AS BEGIN SELECT @p1 END",
    );
    exec(
        &mut engine,
        "CREATE FUNCTION dbo.MyFunc (@f1 INT) RETURNS INT AS BEGIN RETURN @f1 END",
    );

    // 2. Check sys.procedures
    let res = engine
        .query("SELECT name FROM sys.procedures WHERE name = 'MyProc'")
        .unwrap();
    assert_eq!(res.rows.len(), 1, "sys.procedures should contain MyProc");

    // 3. Check sys.functions
    let res = engine
        .query("SELECT name FROM sys.functions WHERE name = 'MyFunc'")
        .unwrap();
    ...
    assert_eq!(
        res.rows.len(),
        2,
        "sys.parameters should contain 2 parameters for MyProc"
    );
    ...
    let res = engine
        .query("SELECT name FROM sys.table_types WHERE name = 'MyTableType'")
        .unwrap();
    assert_eq!(
        res.rows.len(),
        1,
        "sys.table_types should contain MyTableType"
    );

    // 6. Check sys.types for table type
    let res = engine
        .query("SELECT name, is_user_defined FROM sys.types WHERE name = 'MyTableType'")
        .unwrap();
    ...
    assert_eq!(
        res.rows.len(),
        2,
        "sys.columns should contain 2 columns for MyTableType"
    );
}
...
fn test_sys_types_fidelity() {
    ...
    engine
        .exec("CREATE TYPE dbo.TestType AS TABLE (id INT)")
        .unwrap();
    let res = engine
        .query(
            "SELECT name, is_user_defined, system_type_id FROM sys.types WHERE name = 'TestType'",
        )
        .unwrap();
    assert_eq!(res.rows.len(), 1);
