...
#[tokio::test]
#[ignore]
async fn test_null_is_null_compare() {
    let mut client = connect().await;
    let mut engine = Engine::new();

    exec_sql(&mut client, "DROP TABLE IF EXISTS t_null").await;
    exec_sql(&mut client, "CREATE TABLE t_null (id INT, val VARCHAR(10))").await;
    engine_exec(&mut engine, "CREATE TABLE t_null (id INT, val VARCHAR(10))");

    exec_sql(&mut client, "INSERT INTO t_null VALUES (1, NULL), (2, 'A')").await;
    ...
    exec_sql(&mut client, "DROP TABLE t_null").await;
}
...
#[tokio::test]
#[ignore]
async fn test_null_is_not_null_compare() {
    let mut client = connect().await;
    let mut engine = Engine::new();

    exec_sql(&mut client, "DROP TABLE IF EXISTS t_notnull").await;
    exec_sql(&mut client, "CREATE TABLE t_notnull (id INT, val VARCHAR(10))").await;
    engine_exec(&mut engine, "CREATE TABLE t_notnull (id INT, val VARCHAR(10))");

    exec_sql(&mut client, "INSERT INTO t_notnull VALUES (1, NULL), (2, 'A')").await;
    ...
    exec_sql(&mut client, "DROP TABLE t_notnull").await;
}
...
#[tokio::test]
#[ignore]
async fn test_coalesce_null_compare() {
    let mut client = connect().await;
    let mut engine = Engine::new();

    exec_sql(&mut client, "DROP TABLE IF EXISTS t_coal").await;
    exec_sql(&mut client, "CREATE TABLE t_coal (id INT, a VARCHAR(10), b VARCHAR(10))").await;
    engine_exec(&mut engine, "CREATE TABLE t_coal (id INT, a VARCHAR(10), b VARCHAR(10))");

    exec_sql(&mut client, "INSERT INTO t_coal VALUES (1, NULL, 'X'), (2, 'Y', 'Z')").await;
    ...
    for (i, (sql_row, eng_row)) in sql_rows.iter().zip(engine_result.rows.iter()).enumerate() {
        ...
        assert_eq!(sql_row, &eng_strings, "COALESCE linha {} diverge", i);
    }

    exec_sql(&mut client, "DROP TABLE t_coal").await;
}
...
#[tokio::test]
#[ignore]
async fn test_isnull_null_compare() {
    let mut client = connect().await;
    let mut engine = Engine::new();

    exec_sql(&mut client, "DROP TABLE IF EXISTS t_isnull").await;
    exec_sql(&mut client, "CREATE TABLE t_isnull (id INT, val VARCHAR(10))").await;
    engine_exec(&mut engine, "CREATE TABLE t_isnull (id INT, val VARCHAR(10))");

    exec_sql(&mut client, "INSERT INTO t_isnull VALUES (1, NULL), (2, 'A')").await;
    engine_exec(&mut engine, "INSERT INTO t_isnull VALUES (1, NULL), (2, 'A')");

    let sql = "SELECT id, ISNULL(val, 'DEFAULT') AS result FROM t_isnull ORDER BY id";
    let (_, sql_rows) = query_sql(&mut client, sql).await;
    let engine_result = engine_exec(&mut engine, sql).unwrap();
    ...
    for (i, (sql_row, eng_row)) in sql_rows.iter().zip(engine_result.rows.iter()).enumerate() {
        let eng_strings = values_to_strings(eng_row);
        assert_eq!(sql_row, &eng_strings, "ISNULL linha {} diverge", i);
    }

    exec_sql(&mut client, "DROP TABLE t_isnull").await;
}