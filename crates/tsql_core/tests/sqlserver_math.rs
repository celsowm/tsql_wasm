...
async fn test_sign_compare() {
    ...
    for (i, (sql_cell, eng_cell)) in sql_rows[0].iter().zip(values_to_strings(&engine_result.rows[0]).iter()).enumerate() {
        assert_eq!(sql_cell, eng_cell, "SIGN coluna {} diverge", i);
    }
}