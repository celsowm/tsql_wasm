...
fn test_string_agg_with_group_by() {
    ...
    let r = query(&mut e, "SELECT dept, STRING_AGG(name, ';') FROM t GROUP BY dept ORDER BY dept");
...
fn test_string_agg_empty_group_returns_null() {
    ...
    let r = query(&mut e, "SELECT dept, STRING_AGG(val, ',') FROM t WHERE dept = 99 GROUP BY dept");
