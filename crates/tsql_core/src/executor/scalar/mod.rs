...
pub(crate) fn eval_function(
    ...
    match name.to_uppercase().as_str() {
        ...
        "CEILING" => math::eval_math_unary(args, row, ctx, catalog, storage, clock, "CEILING", |f| f.ceil()),
        "FLOOR" => math::eval_math_unary(args, row, ctx, catalog, storage, clock, "FLOOR", |f| f.floor()),
        ...
        "EDIT_DISTANCE_SIMILARITY" => eval_edit_distance_similarity(args, row, ctx, catalog, storage, clock),
        "JARO_WINKLER_DISTANCE" => eval_jaro_winkler_distance(args, row, ctx, catalog, storage, clock),
        "JARO_WINKLER_SIMILARITY" => eval_jaro_winkler_similarity(args, row, ctx, catalog, storage, clock),
        "CURRENT_TIMESTAMP" => {
            if !args.is_empty() {
                return Err(DbError::Execution("CURRENT_TIMESTAMP expects no arguments".into()));
            }
            Ok(Value::DateTime(clock.now_datetime_literal()))
        }
        "CURRENT_DATE" => {
        ...
        "@@VERSION" => Ok(Value::NVarChar("Microsoft SQL Server 2022 (RTM) - 16.0.1000.6 (tsql_wasm emulator)".into())),
        ...
        "COUNT" | "SUM" | "AVG" => Err(DbError::Execution(format!("{} is only supported in grouped projection", name))),
        "MIN" | "MAX" => Err(DbError::Execution("MIN/MAX require a FROM clause when used in scalar context (use in GROUP BY)".into())),
        "COUNT_BIG" => Err(DbError::Execution("COUNT_BIG is only supported in grouped projection".into())),
...
fn eval_json_value(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.len() != 2 { return Err(DbError::Execution("JSON_VALUE expects 2 arguments".into())); }
    let json_val = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    let path_val = eval_expr(&args[1], row, ctx, catalog, storage, clock)?;
    if json_val.is_null() { return Ok(Value::Null); }
    json::json_value(&json_val.to_string_value(), &path_val.to_string_value())
}

fn eval_json_query(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.len() != 2 { return Err(DbError::Execution("JSON_QUERY expects 2 arguments".into())); }
    let json_val = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    let path_val = eval_expr(&args[1], row, ctx, catalog, storage, clock)?;
    if json_val.is_null() { return Ok(Value::Null); }
...
fn eval_json_modify(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.len() != 3 { return Err(DbError::Execution("JSON_MODIFY expects 3 arguments".into())); }
    let json_val = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    let path_val = eval_expr(&args[1], row, ctx, catalog, storage, clock)?;
    let new_val = eval_expr(&args[2], row, ctx, catalog, storage, clock)?;
    if json_val.is_null() { return Ok(Value::Null); }
    json::json_modify(&json_val.to_string_value(), &path_val.to_string_value(), &new_val.to_string_value())
}

fn eval_isjson(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.len() != 1 { return Err(DbError::Execution("ISJSON expects 1 argument".into())); }
    let json_val = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    if json_val.is_null() { return Ok(Value::Null); }
    json::is_json(&json_val.to_string_value())
}

fn eval_json_array_length(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.len() != 1 { return Err(DbError::Execution("JSON_ARRAY_LENGTH expects 1 argument".into())); }
    let json_val = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    if json_val.is_null() { return Ok(Value::Null); }
    json::json_array_length(&json_val.to_string_value())
}

fn eval_json_keys(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.is_empty() || args.len() > 2 { return Err(DbError::Execution("JSON_KEYS expects 1 or 2 arguments".into())); }
    let json_val = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    if json_val.is_null() { return Ok(Value::Null); }
...
fn eval_regexp_like(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.len() < 2 || args.len() > 3 { return Err(DbError::Execution("REGEXP_LIKE expects 2 or 3 arguments".into())); }
    let s_val = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    let p_val = eval_expr(&args[1], row, ctx, catalog, storage, clock)?;
    if s_val.is_null() || p_val.is_null() { return Ok(Value::Null); }
    let flags = if args.len() == 3 { Some(eval_expr(&args[2], row, ctx, catalog, storage, clock)?.to_string_value()) } else { None };
    regexp::regexp_like(&s_val.to_string_value(), &p_val.to_string_value(), flags.as_deref())
}

fn eval_regexp_replace(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.len() < 3 || args.len() > 4 { return Err(DbError::Execution("REGEXP_REPLACE expects 3 or 4 arguments".into())); }
    let s_val = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    let p_val = eval_expr(&args[1], row, ctx, catalog, storage, clock)?;
    let r_val = eval_expr(&args[2], row, ctx, catalog, storage, clock)?;
    if s_val.is_null() || p_val.is_null() || r_val.is_null() { return Ok(Value::Null); }
    let flags = if args.len() == 4 { Some(eval_expr(&args[3], row, ctx, catalog, storage, clock)?.to_string_value()) } else { None };
    regexp::regexp_replace(&s_val.to_string_value(), &p_val.to_string_value(), &r_val.to_string_value(), flags.as_deref())
}

fn eval_regexp_substr(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.len() < 2 || args.len() > 5 { return Err(DbError::Execution("REGEXP_SUBSTR expects 2 to 5 arguments".into())); }
    let s_val = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    let p_val = eval_expr(&args[1], row, ctx, catalog, storage, clock)?;
    if s_val.is_null() || p_val.is_null() { return Ok(Value::Null); }
    let pos = if args.len() >= 3 { eval_expr(&args[2], row, ctx, catalog, storage, clock)?.to_string_value().parse::<usize>().unwrap_or(1) } else { 1 };
    let occurrence = if args.len() >= 4 { eval_expr(&args[3], row, ctx, catalog, storage, clock)?.to_string_value().parse::<usize>().unwrap_or(0) } else { 0 };
    let flags = if args.len() == 5 { Some(eval_expr(&args[4], row, ctx, catalog, storage, clock)?.to_string_value()) } else { None };
    regexp::regexp_substr(&s_val.to_string_value(), &p_val.to_string_value(), pos, occurrence, flags.as_deref())
}

fn eval_regexp_instr(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.len() < 2 || args.len() > 6 { return Err(DbError::Execution("REGEXP_INSTR expects 2 to 6 arguments".into())); }
    let s_val = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    let p_val = eval_expr(&args[1], row, ctx, catalog, storage, clock)?;
    if s_val.is_null() || p_val.is_null() { return Ok(Value::Null); }
    let pos = if args.len() >= 3 { eval_expr(&args[2], row, ctx, catalog, storage, clock)?.to_string_value().parse::<usize>().unwrap_or(1) } else { 1 };
    let occurrence = if args.len() >= 4 { eval_expr(&args[3], row, ctx, catalog, storage, clock)?.to_string_value().parse::<usize>().unwrap_or(0) } else { 0 };
    let return_opt = if args.len() >= 5 { eval_expr(&args[4], row, ctx, catalog, storage, clock)?.to_string_value().parse::<usize>().unwrap_or(0) } else { 0 };
    let flags = if args.len() == 6 { Some(eval_expr(&args[5], row, ctx, catalog, storage, clock)?.to_string_value()) } else { None };
    regexp::regexp_instr(&s_val.to_string_value(), &p_val.to_string_value(), pos, occurrence, return_opt, flags.as_deref())
}

fn eval_regexp_count(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.len() < 2 || args.len() > 4 { return Err(DbError::Execution("REGEXP_COUNT expects 2 to 4 arguments".into())); }
    let s_val = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    let p_val = eval_expr(&args[1], row, ctx, catalog, storage, clock)?;
    if s_val.is_null() || p_val.is_null() { return Ok(Value::Null); }
    let pos = if args.len() >= 3 { eval_expr(&args[2], row, ctx, catalog, storage, clock)?.to_string_value().parse::<usize>().unwrap_or(1) } else { 1 };
    let flags = if args.len() == 4 { Some(eval_expr(&args[3], row, ctx, catalog, storage, clock)?.to_string_value()) } else { None };
    regexp::regexp_count(&s_val.to_string_value(), &p_val.to_string_value(), pos, flags.as_deref())
}

// Delegate Fuzzy functions
fn eval_edit_distance(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.len() != 2 { return Err(DbError::Execution("EDIT_DISTANCE expects 2 arguments".into())); }
    let s1 = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    let s2 = eval_expr(&args[1], row, ctx, catalog, storage, clock)?;
    if s1.is_null() || s2.is_null() { return Ok(Value::Null); }
    Ok(Value::Int(fuzzy::edit_distance(&s1.to_string_value(), &s2.to_string_value())))
}

fn eval_edit_distance_similarity(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.len() != 2 { return Err(DbError::Execution("EDIT_DISTANCE_SIMILARITY expects 2 arguments".into())); }
    let s1 = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    let s2 = eval_expr(&args[1], row, ctx, catalog, storage, clock)?;
    if s1.is_null() || s2.is_null() { return Ok(Value::Null); }
...
fn eval_jaro_winkler_distance(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.len() != 2 { return Err(DbError::Execution("JARO_WINKLER_DISTANCE expects 2 arguments".into())); }
    let s1 = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    let s2 = eval_expr(&args[1], row, ctx, catalog, storage, clock)?;
    if s1.is_null() || s2.is_null() { return Ok(Value::Null); }
...
fn eval_jaro_winkler_similarity(args: &[Expr], row: &[ContextTable], ctx: &mut ExecutionContext, catalog: &dyn Catalog, storage: &dyn Storage, clock: &dyn Clock) -> Result<Value, DbError> {
    if args.len() != 2 { return Err(DbError::Execution("JARO_WINKLER_SIMILARITY expects 2 arguments".into())); }
    let s1 = eval_expr(&args[0], row, ctx, catalog, storage, clock)?;
    let s2 = eval_expr(&args[1], row, ctx, catalog, storage, clock)?;
    if s1.is_null() || s2.is_null() { return Ok(Value::Null); }
