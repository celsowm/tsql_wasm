...
use tsql_core::{Database, SessionId, SessionManager};
...
impl SessionPool {
    ...
    pub fn ensure_min_sessions(&self, db: &Database) {
    ...
    pub fn checkout(&self, db: &Database) -> Result<SessionId, CheckoutError> {
        let now = Instant::now();
