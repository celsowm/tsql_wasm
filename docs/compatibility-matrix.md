...
| `PIVOT` and `UNPIVOT` | compatible subset | Fully implemented in parser and executor; SUM, AVG, COUNT, MIN, MAX, COUNT_BIG, STRING_AGG aggregates supported; NULL handling in UNPIVOT matches SQL Server (skips NULLs); tests added in Phase 1 | `crates/tsql_core/src/parser/parse/statements/query.rs`, `crates/tsql_core/src/executor/query/transformer/pivot.rs`, `crates/tsql_core/src/executor/query/transformer/unpivot.rs`, `crates/tsql_core/tests/phase1_pivot.rs`, `crates/tsql_core/tests/phase1_unpivot.rs` |
