...
| Procedural T-SQL (`DECLARE`, `SET`, `IF`, `WHILE`, `TRY/CATCH`) | compatible subset | Broad procedural subset is present; `SET ANSI_DEFAULTS` now expands to correct constituent flags | `README.md`, `crates/iridium_core/tests/phase4_programmability.rs`, `crates/iridium_core/tests/try_catch_test.rs`, `crates/iridium_core/tests/metadata_fidelity.rs` |
...
| Session and Connection built-ins | compatible subset | Support for `CONNECTIONPROPERTY`, `SESSIONPROPERTY` | `crates/iridium_core/src/executor/scalar/system/properties.rs` |
...
| `INFORMATION_SCHEMA` domain views (`COLUMN_DOMAIN_USAGE`, `DOMAINS`, `DOMAIN_CONSTRAINTS`) | compatible subset | Backed by table types; `DOMAIN_CONSTRAINTS` moved to types module to match SQL Server schema | `crates/iridium_core/src/executor/metadata/info_schema_types.rs`, `crates/iridium_core/tests/metadata_fidelity.rs` |
| Core `sys.*` catalog views | compatible subset | Broad coverage exists including `sys.views`, `sys.database_principals`, `sys.database_permissions`, `sys.database_role_members`, `sys.index_columns` (with `key_ordinal`), `sys.procedures` (with `is_ms_shipped`), `sys.filegroups` (with PRIMARY row), `sys.all_objects`, `sys.identity_columns`, `sys.computed_columns`, `sys.sql_expression_dependencies` (empty stub), `sys.dm_os_sys_info`, `sys.dm_exec_requests`, `sys.dm_db_index_usage_stats`, `sys.dm_db_partition_stats`; partition functions/schemes still return empty rows | `README.md`, `crates/iridium_core/src/executor/metadata/sys/*`, `crates/iridium_core/src/executor/metadata/sys/partition.rs`, `crates/iridium_core/tests/metadata_fidelity.rs` |
