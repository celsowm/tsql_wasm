...
It makes it more *disciplined*. Managing complex state like a buffer pool, transaction logs, and concurrent lock tables requires careful ownership design. Rust forces you to handle these patterns safely, which pays off in fewer production crashes.
...
Yes, TVPs are supported both in T-SQL batches and via the TDS protocol, including multi-column TVPs and `READONLY` enforcement.
...
Both simple and recursive CTEs are supported. The engine handles the recursive anchor and recursive members according to T-SQL semantics.
...
The engine implements T-SQL's implicit conversion rules. For example, it can automatically coerce `DATE` to `DATETIME2` or `BINARY(16)` to `UNIQUEIDENTIFIER`.

### 28. Is there a query optimizer?
The engine currently uses a rule-based approach for most queries. However, it does support index seeks and range scans when appropriate indexes are available. A more advanced cost-based optimizer is on the long-term roadmap.
...
The engine includes a WAL implementation for crash recovery. It records transaction changes before they are applied to the main data files, ensuring ACID guarantees even after a power failure.

### 32. How are indexes stored?
Indexes are implemented using B-Trees (provided by `redb` or our own BTree implementation depending on the configuration). They store key-to-RID (Row ID) mappings to allow for fast lookups.
...
We support `READ COMMITTED`, `READ UNCOMMITTED`, and `SNAPSHOT` isolation. The implementation uses a mix of row-level locks and MVCC (Multi-Version Concurrency Control) depending on the requested level.
...
Yes. The engine includes a deadlock detector that builds a "wait-for" graph. If a cycle is detected, one of the sessions is chosen as a "victim," its transaction is rolled back, and a 1205 error is returned.
...
In the browser, persistence is "memory-first" by default. However, Iridium SQL provides `exportCheckpoint()` and `fromCheckpoint()` methods. You can save the exported byte array to `IndexedDB` or `localStorage` and restore it when the user returns.
...
Yes, the server supports TLS for secure connections. You can provide your own certificate or use a self-signed one for development.
...
Please open an issue on our GitHub repository with a reproducible T-SQL script and a description of the expected vs. actual behavior.
